...
def jieba_cut(text):
    ...
    :return: 符合规则的分词结果
    '''
    # 只保留名动词、动词、时间词、其他名词、人名、地名、名词、习用语、简称略语、成语
    rule_words = ['vn', 'v', 't', 'nz', 'nr', 'ns', 'n', 'l', 'i', 'j']
    # 自定义stop_words
    stop_words = ['搜狐','是','人','有','说','标题','搜狐','转','登录']
    words = pseg.cut(text)  # 分词
    seg_list = [word.word for word in words if word.flag in rule_words and word.word not in stop_words]  # 列表用于存储每个文件的分词结果
    return seg_list
...
def data_parse(data):
    ...
    content_list = [str_convert(each_content.text) for each_content in content_code if
                    len(each_content) > 0]  # 将转换后的content文本内容加入列表
    return content_list
...
num_topics = 3  # 设置主题个数
lda = models.LdaModel(corpus_tfidf, id2word=dic, num_topics=num_topics, random_state=0)  # 通过LDA进行主题建模
